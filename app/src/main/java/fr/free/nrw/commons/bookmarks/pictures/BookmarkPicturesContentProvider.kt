...
class BookmarkPicturesContentProvider : CommonsDaggerContentProvider() {
    ...
    override fun update(
        ...
        val rowsUpdated = if (selection.isNullOrEmpty()) {
            ...
            sqlDB.update(TABLE_NAME, values, "$COLUMN_MEDIA_NAME = ?", arrayOf(id.toString()))
        } else {
    ...
    override fun delete(uri: Uri, selection: String?, selectionArgs: Array<out String>?): Int {
        ...
        val rows = db.delete(TABLE_NAME, "media_name = ?", arrayOf(uri.lastPathSegment))
        context!!.contentResolver.notifyChange(uri, null)
        return rows
    }
    ...
    companion object {
        ...
        val BASE_URI: Uri = Uri.parse("content://${BuildConfig.BOOKMARK_AUTHORITY}/$BASE_PATH")
        const val TABLE_NAME = BookmarkPicturesDao.Table.TABLE_NAME
        const val COLUMN_MEDIA_NAME = BookmarkPicturesDao.Table.COLUMN_MEDIA_NAME

        fun uriForName(name: String): Uri = Uri.parse("${BASE_URI}/$name")
    }
}
